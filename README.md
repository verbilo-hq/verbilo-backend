...
6. `npm run seed` — idempotent. Upserts four cross-sector demo tenants (SmileCo Dental Group / Riverside Vets / BrightSight Opticians / Greenfield GP Federation) plus their sites. If you also set `SEED_USERNAME` and `SEED_COGNITO_SUB` env vars, it pairs a User row to one of them (defaults to SmileCo; override with `SEED_TENANT_SLUG`). Optionally set `SEED_ROLE` to one of the values in `src/common/user-roles.ts` (e.g. `verbilo_super_admin`) to stamp the User's role at the same time — handy for provisioning internal admin accounts.
